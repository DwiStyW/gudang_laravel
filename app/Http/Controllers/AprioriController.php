...
class AprioriController extends Controller
{
    ...
    public function index(){
        ...
        $dataform=DB::select('SELECT noform,count(*) From riwayat where noform!="saldo_awal"
        and keluar!=0 group by noform');
        foreach($dataform as $t){
            $noform=$t->noform;
            $tr=DB::select("SELECT noform,kode from riwayat where noform='$noform'
            group by noform,kode");
            $gdform[]=$tr;
        }
        ...
        return view('apriori.apriori',compact('gdform','master','transaksi'));
    }
}